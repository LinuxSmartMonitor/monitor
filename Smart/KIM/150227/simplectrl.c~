...
printf("	i'll tranfer you framebuffer\n");
   for (y = 0; y < vinfo.yres; y++)	{
			trans_count = 0;		// INIT trans_count - HHH
         for (x = 0; x < vinfo.xres; x++) {
             location = (x+vinfo.xoffset) * (vinfo.bits_per_pixel/8) +
					...
					*((unsigned short int*)(fbp + location)) = 31;	//*** ALL RED TEST			
					...
					//if(response<0) {error(response); return -1;}
						...
						libusb_bulk_transfer(handle,OUT,trans, 1024 * 2, &transferred,0);
						printf("RECEIVED\n");
						break;	//*** only one line TEST
	}	//y
